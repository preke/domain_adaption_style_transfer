        ...
        for batch in train_iter:
            ...
            if cnt_batch % 2000 == 0:                
                ...
                if flag:
                    ...
                    logger.info('epoch: %d, [iter: %d / all %d], err_s_label: %f, err_t_domain: %f, err_ae: %f' \
                      % (epoch, cnt_batch, len_iter, err_label.cpu().data.numpy(), out, reconstruct_loss))
...
def show_reconstruct_results(dev_iter, model, args, cnt=0, reconstruct_loss=0.0):
    writer = open('res/rec_logs_'+str(cnt)+'_' + str(float(reconstruct_loss)) + '_.txt', 'w')
    cnt_batch = 0
    for batch in dev_iter:
